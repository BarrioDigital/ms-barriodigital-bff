/**
 * Procesa y valida el envelope de mensaje común
 */
function parseMessageEnvelope(msg) {
  const content = JSON.parse(msg.content.toString());
  
  // Soporte para carga útil directa o mediante envelope común del sistema
    ...
    channel.consume(q.name, (msg) => {
      if (msg !== null) {
        try {
          ...
          // ACK explícito al procesar con éxito
          channel.ack(msg);
        } catch (error) {
          console.error(`[Consumer Error] Fallo al procesar mensaje en ${q.name}:`, error.message);
          // NACK explícito sin reencolamiento directo para dirigir a DLQ
